...
Total_sales = df2[df2['Total_sales'] == df2['Total_sales'].max()]
df_pct_change = df2[['Product_A', 'Product_B', 'Product_C']].pct_change() * 100
df_pct_change['Date'] = df2['Date']
...
avg_salary_by_dept = df3.groupby('Department')['Salary'].mean().reset_index()
...
print(f"Average Quantity Ordered: {average_quantity:.2f}")
